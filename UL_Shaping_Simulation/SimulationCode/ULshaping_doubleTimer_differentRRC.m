...
%% ======Inputs======
timeObservation = 10^5;         % (ms)
numSim = 1;
%======Traffic model (Background application update)======
%Type in the period of the update periods of applications in ms.
periodApp = [200 300 500 600 1200]; % (ms)
...
% rrcReleaseTimer = [0 100 500 1000 2500 5000 10000 15000 30000 60000];%% (ms): 0s, 0.1s, 0.5s, 1s, 2.5s, 5s,10s,15s,30s, 60s, 
rrcReleaseTimer = 100;
delayIdleToConn = 50;           % (ms)
%======DRX configuration======
inactivityTimer = 4;
onDuration = 2;
% drxCycle = [5 20 128 256];
drxCycle = 40;
%======shaping parameter setting======
% On/off shaping mechanism timer in ms time scale.
T1 = 100*linspace(1,10,10);   %ms
T2 = 100*linspace(1,10,10);   %ms
...
%%  ======Simulation starts======
for n = 1:1:numSim
    for row = 1:1:length(T1)
        for col = 1:1:length(T2)
            %% Traffic generation (Background application update)
            % periodLCM = 1;
            ...
            numPktArrival = zeros(1,timeObservation);
            
            for timeBGTraffic = 1:1:timeObservation
                for p = 1:1:numel(periodApp)
                    if mod(timeBGTraffic,periodApp(p)) == 0
                        numPktArrival(timeBGTraffic) = numPktArrival(timeBGTraffic)+1;
                    end
                end
            end
            
            % Retrieve orginal packet arrival time.
            pktArrivalTime = find(numPktArrival);
            if max(numPktArrival)>=2
                for tp = 2:1:max(numPktArrival)
                    tp_count = find(numPktArrival >= tp);
                    for tpp = 1:1:length(tp_count)
                        arritime = tp_count(tpp);
                        loc = find(pktArrivalTime==arritime);
                        loc = loc(end);
                        pktArrivalTime = [pktArrivalTime(1:loc) arritime pktArrivalTime(loc+1:end)];
                    end
                end
            end
            
            % T1 & T2 shaping timer.
            shapingTimerInterval = T1(row)+T2(col);
            for timeShapingTraffic = 1:1:(timeObservation - shapingTimerInterval)
                if mod(timeShapingTraffic,shapingTimerInterval) > T1(row) && numPktArrival(timeShapingTraffic) > 0
                    shiftTime = timeShapingTraffic + T2(col) - (mod(timeShapingTraffic,shapingTimerInterval) - T1(row));
                    numPktArrival(shiftTime) = numPktArrival(shiftTime) + numPktArrival(timeShapingTraffic);
                    numPktArrival(timeShapingTraffic) = 0;
                end
            end
            
            % Retrieve shaped packet arrival time.
            pktArrivalTime_shaped = find(numPktArrival);
            if max(numPktArrival)>=2
                for tp = 2:1:max(numPktArrival)
                    tp_count = find(numPktArrival >= tp);
                    for tpp = 1:1:length(tp_count)
                        arritime = tp_count(tpp);
                        loc = find(pktArrivalTime_shaped==arritime);
                        loc = loc(end);
                        pktArrivalTime_shaped = [pktArrivalTime_shaped(1:loc) arritime pktArrivalTime_shaped(loc+1:end)];
                    end
                end
            end
            ...
            pktArrivalTime_copy = pktArrivalTime;
            ...
            %%
            while length(pktDepartureTime) ~= length(pktArrivalTime)
                ...
                while strcmp(state,'Active')
                    ...
                    while ~isempty(bufferedPktSize) || (~isempty(pktArrivalTime_copy) && pktArrivalTime_copy(1) < timeLeaveState)     % pkt transmission (EFB + BP's)
                        ...
                        while ~isempty(pktArrivalTime_copy) && pktArrivalTime_copy(1) < timeCurrent      %!! handle 2 situations of nwe arrivals: (1) If buffer is not empty: pkt arrive at the timeCurrent should be appended after the buffered data (those arrive at timeCurrent can't be served at the same time)
...
%legend('C_R = 10     (ms)', 'C_R = 100   (ms)', 'C_R = 500   (ms)', 'C_R = 1000 (ms)');
xlabel('shaping timer T1 (ms)');
% xlabel('delay bound constraint (ms)');
% xlabel('buffer size (number of packets)');
