temp = xlsread('VoIP_Skype.xlsx', 'Total_CDF');
VoIP_Skype_Size = temp(:,3);
VoIP_Skype_IAT = temp(1:length(temp)-1,5);

temp = xlsread('VoIP_Skype.xlsx', 'Uplink_CDF');
VoIP_Skype_UL_Size = temp(:,3);
VoIP_Skype_UL_IAT = temp(1:length(temp)-1,5);

temp = xlsread('VoIP_Skype.xlsx', 'Downlink_CDF');
VoIP_Skype_DL_Size = temp(:,3);
VoIP_Skype_DL_IAT = temp(1:length(temp)-1,5);

figure
hold on

p = cdfplot(VoIP_Skype_IAT);
set(p,'Color',[0 0.6 0.2],'LineWidth',2);

p = cdfplot(VoIP_Skype_UL_IAT);
set(p,'Color',[0.2 0.8 0.3],'LineWidth',2);

p = cdfplot(VoIP_Skype_DL_IAT);
