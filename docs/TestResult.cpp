...
int main(){
	...
	for ( i =0; i < lengthX; i++ ){
		...
		if (X[i] != 0){
			if ( X[i] > threshold ){
				calculatedPower = alphaH*( P0 + 10*log(X[i]) + alpha*PL) + betaH;
			}
			else{
				calculatedPower = alphaL*( P0 + 10*log(X[i]) + alpha*PL) + betaL;
			}
			if (calculatedPower<0)
				calculatedPower = 0;
			if (calculatedPower>24)
				calculatedPower = 24;
			originalPower = originalPower + calculatedPower;
		}
	} 
	cout << "}" << endl;
		...
		if ( X[i] != 0 ){
			if ( X[i] > threshold ){
				...
				if (calculatedPower<0)
					...
					calculatedPower = 24;
				modifiedPower = modifiedPower + calculatedPower;
			}
			else{
				calculatedPower = alphaL*( P0 + 10*log(X[i]) + alpha*PL) + betaL;
				if (calculatedPower<0)
					...
					calculatedPower = 24;
				modifiedPower = modifiedPower + alphaL*( P0 + 10*log(X[i]) + alpha*PL) + betaL;
			}
		}
	}
	cout << "}" << endl;
